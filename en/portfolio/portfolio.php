<?php
function render($str) {
    $files = glob(__DIR__. "/../../portfolio/photos/{$str}/*");
    $total = count($files) / 2;
    preg_match('/\.[a-z]+$/', $files[0], $format);
    $format = $format[0];
    $result = [];
    foreach (range(0, $total - 1) as $i) {
        $result[] = "<img alt=\"{$str} photo\" class=\"portfolio__{$str}_photo\" src=\"/portfolio/photos/{$str}/${i}-minify{$format}\">";
    }
    shuffle($result);
...
<div class="row justify-content-center">
    <div class="col-md-10">
        <ul class="navigation nav nav-tabs">
            <li class="nav-item">
                <a class="navigation__individual nav-link <?= $link === 'individual' ? 'active' : '' ?>" style="cursor: pointer">Individual</a>
            </li>
            <li class="nav-item">
                <a class="navigation__pair nav-link <?= $link === 'pair' ? 'active' : '' ?>" style="cursor: pointer">Love-story</a>
            </li>
            <li class="nav-item">
                <a class="navigation__beauty nav-link <?= $link === 'beauty' ? 'active' : '' ?>" style="cursor: pointer">Beauty</a>
            </li>
            <li class="nav-item">
                <a class="navigation__retouch nav-link <?= $link === 'retouch' ? 'active' : '' ?>" style="cursor: pointer">Retouch</a>
            </li>
        </ul>
    </div>
</div>
            ...
            <div class="portfolio__container">
                ...
                <div class="portfolio__retouch">
                    <div class="portfolio__retouch_photos">
                        ...
                        $result = '';
                        foreach (range(0, $total - 1) as $i) {
                            $result .= '<div class="portfolio__retouch_photo"><img alt="Photo before retouch" src="" /><img alt="Photo after retouch" src="" /></div>';
                        }
                        echo $result;
                        ?>
                    </div>
